...
if isinstance(df.columns, pd.MultiIndex):
    st.warning("Flattening multi-index columns.")
    df.columns = ['_'.join(col) for col in df.columns]
# Ensure column names are consistent
st.write("Columns after flattening:", df.columns)

# Map multi-ticker columns to single-ticker format
# Example: Rename `Open_MMM` to `Open`, `Close_MMM` to `Close`, etc.
...
if not required_columns.issubset(df.columns):
    st.error(
        ...
        f"{', '.join(required_columns)}. Found columns: {', '.join(df.columns)}"
    )
else:
    st.success("The DataFrame has valid columns for technical analysis.")
